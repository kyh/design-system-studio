...
import componentHierarchy from "./branding/documentation/component-hierarchy.svg";

<Meta title="Guides/Creating Components" />

# Creating Components

Components allow you to split the UI into independent and reusable pieces. However, not all components are built the same way. A component might be created using styled-components or a combination of styled-components and Styled System or as a composition of other components. DS uses the following hierarchy to reason as to what technique to use for creating components.

<img style={{ margin: 32 }} src={componentHierarchy} />
...
Each DSS UI component returns a single HTML element that accepts all HTML props, including className, style and accessibility attributes.
...
DSS UI comes with many patterns that you can use in your application either standalone or as part of your feature. In the case you'd like to create a new pattern using DSS UI' approach, here is a sample breakdown of a DSS UI pattern
