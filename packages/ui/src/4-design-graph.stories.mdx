...
import { fonts, fontSizes, fontWeights, lineHeights } from "./theme/tokens";
import { Flex, Box, Image, FormControl, Label, Input } from "./components";

<Meta title="Guides/Design Graph" />
...
They define the visual characteristics of a brand or a product such as typography, colors, icons, and spacing. They make it easier to maintain a scalable, consistent system for UI development. DSS UI maintains all design tokens in a `theme.ts` file.
 ...
 */
<Box color="textEditorial" bg="page.background" p={3}>
  Tomato
</Box>
```
