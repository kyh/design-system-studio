...
The build process is configured to transform SVGs into React components, using [SVGR](https://react-svgr.com). The script looks for all `.svg` files in `packages/ui/src/components/icons` and generates a `.tsx` file of the same name.
        ...
        return (
          <Flex key={name} alignItems="center">
            <Component
              ...
              aria-hidden
            />
            <Text ml={2} fontSize="body" color="text.secondary">
              {name}
            </Text>
          </Flex>
        );
...
```js
import { Icons } from "@dss/ui";
```

<Preview withSource="open">
  <Story name="usage">
    <Icons.CheckboxChecked title="checkbox checked" color="page.color" />
  </Story>
</Preview>
...
<Preview>
  <Story name="sizing with font-size">
    <Flex alignItems="center">
      <Icons.Loader fontSize="body" mr={2} color="page.color" aria-hidden />
      <Icons.Loader fontSize="heading" mr={2} color="page.color" aria-hidden />
    </Flex>
  </Story>
</Preview>
...
<Preview>
  <Story name="sizing with layout props">
    <Icons.Loader width={3} height={3} color="page.color" aria-hidden />
  </Story>
</Preview>
...
Add or replace `.svg` files in `packages/ui/src/components/icons`. The design system comes pre-populated with a few icons, which must either be preserved or replaced with similar icons.
...
<Preview withSource="open">
  <Story name="as decorative">
    <Icons.RadioChecked color="page.color" aria-hidden />
  </Story>
</Preview>
...
<Preview withSource="open">
  <Story name="as non-decorative">
    <Icons.Loader color="page.color" title="waiting for content to load" />
  </Story>
</Preview>
